...
| 2 | 测试工坊 / Test workshop | 0–32 全僵尸目录保留；Beta 2 禁用生成、扶梯与清理按钮 / all zombie IDs remain cataloged; spawn, ladder, and cleanup buttons are disabled in Beta 2 |
...
- Night Roof Endless is explicitly labeled an experiment, not an original hidden mode. Beta 2 keeps its button disabled pending live verification of internal calls.
...
This repository currently builds **1.0 Beta 2** (`1.0.0-beta.2`). It recognizes the allow-listed official Steam wrapper runtime while keeping internal game calls disabled. Features that touch live game state remain behind exact version, runtime, byte, capacity, and ownership checks. See [LIVE_TEST_CHECKLIST.md](LIVE_TEST_CHECKLIST.md).
