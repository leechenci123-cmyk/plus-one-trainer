...
- Advanced Pause is verified against the Steam 1096 runtime: world updates stop while planting, shoveling, seed selection, and Cob Cannon input remain available. The button, `F3`, and focus-loss pause are enabled on that runtime.
...
- Night Roof is honestly labeled a scene-mixing experiment—not an alleged hidden original mode—and its entry point remains guarded.
...
- A test workshop catalogs zombie IDs `0–32`, including Yeti and both Gargantuars. Spawn and ladder calls enforce scene, capacity, and object-identity checks.
...
Unknown versions stay read-only. Beta 5 first verifies the Steam wrapper parent's complete disk hash, then connects only to the `popcapgame1.exe` child launched by that parent. The child must also pass x86, timestamp, image-base, and live-object checks.

## Use

1. Download and extract `Plus-One-Trainer-1.0.0-beta.5-win-x86.zip` from GitHub Releases.
...
This is **1.0 Beta 5** (technical version `1.0.0-beta.5`). It adds wallet money `+1,000`, credits auto-collected items through the native collection path, and enables the verified Advanced Pause and workshop paths. Test challenge rules one setting at a time after a backup; do not promote to stable `1.0.0` until the [live-test checklist](docs/LIVE_TEST_CHECKLIST.md) passes.
...
./scripts/test.ps1
./scripts/package.ps1 -Version 1.0.0-beta.5
```
